...
const StudentsHonorBoardCarousel = () => {
  ...
  useEffect(() => {
    if (swiperRef.current) {
      swiperRef.current.changeLanguageDirection(isArabic ? 'rtl' : 'ltr');
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4">
        ...
        {/* Navigation Buttons */}
        {students.length > 0 && (
          <div className={`flex justify-end items-center gap-4 mb-6 transition-all duration-700 delay-300 ${
            isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
          }`}>
            <Button
              variant="outline"
              size="icon"
              onClick={handlePrev}
              className="rounded-full w-12 h-12 hover:scale-110 transition-transform duration-300"
            >
              ...
              {students.map((student, index) => {
                ...
                return (
                  <SwiperSlide key={student.id}>
                    <Card className="group cursor-pointer hover:shadow-2xl transition-all duration-500 border-0 shadow-lg relative overflow-hidden h-full mx-2">
                      {/* Rank Badge */}
                        ...
                        <div className="grid grid-cols-3 gap-4 text-center">
                          ...
                          <div className="transform transition-all duration-300 group-hover:scale-110">
                            <div className="text-lg font-bold text-amber-600 flex flex-col items-center gap-1">
                              ...
                              {student.average_rating?.toFixed(1) || '0.0'}
                            </div>
                            <div className="text-xs text-muted-foreground transition-all duration-300 group-hover:text-foreground group-hover:font-medium">
                          ...
                          <div className="text-center">
                            ...
                            <div className="text-2xl font-bold text-green-600">
                              {student.total_courses > 0 
                                ? Math.round((student.completed_courses / student.total_courses) * 100)
                                : 0
                              }%
                            </div>
                          </div>
