...
interface Course {
  ...
  active: boolean;
  max_students?: number; // ✅ إضافة الحد الأقصى للطلاب
  teacher: {
    id: number;
    name: string;
    email: string;
...
const Courses = () => {
  ...
  // ✅ دالة محسنة لتحويل البيانات مع إضافة maxStudents
  const transformCourseData = (course: Course) => {
    ...
    // ✅ تحديد الحد الأقصى للطلاب بناءً على نوع الكورس
    const getMaxStudents = () => {
      // إذا كان موجود في البيانات من الـ API
      if (course.max_students) return course.max_students;
      
      // إذا كان الكورس جماعي، نضع حد أقصى مناسب
      if (course.course_type === 'group') {
        // يمكنك ضبط هذه القيم حسب احتياجاتك
        if (course.count_student && course.count_student > 50) return 100;
        if (course.count_student && course.count_student > 25) return 50;
        return 30; // الحد الأدنى للكورسات الجماعية
      }
      
      // الكورسات الخاصة ليس لها حد أقصى
      return undefined;
    };

    // ✅ حساب التقدم بناءً على عدد الطلاب الحاليين والحد الأقصى
    const getEnrollmentProgress = () => {
      const maxStudents = getMaxStudents();
      if (!maxStudents) return 0; // لا يوجد progress للكورسات الخاصة
    ...
    return {
      ...
      // ✅ بيانات الـ Progress Bar المحسنة
      maxStudents: maxStudents, // ✅ تمرير الحد الأقصى
      enrollmentProgress: enrollmentProgress, // ✅ تمرير نسبة التقدم
